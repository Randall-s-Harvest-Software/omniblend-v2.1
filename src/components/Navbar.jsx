...
const Navbar = () => {
    ...
    return (
        <>
            ...
            {/* Main Navbar */}
            <nav
                className={`w-full max-w-[340px] md:max-w-[90%] lg:max-w-[85%] xl:max-w-[80%] 2xl:max-w-[70%] h-[45px] md:h-[60px] flex items-center px-4 sm:px-6 md:px-8 bg-white/5 backdrop-blur-md border border-white/10 rounded-full shadow-2xl mx-auto fixed top-[20px] sm:top-[25px] md:top-[30px] left-1/2 transform -translate-x-1/2 z-50 ${
            ...
            >
                ...
                <div className={`flex items-center ${isAuthPage ? "flex-1 justify-center" : ""}`}>
                    <img src="/logo1.png" alt="Logo" className="w-[80px] h-auto" />
                </div>
...
const NavLink = ({ to, children, mobile = false }) => (
    <Link
        to={to}
        className={`${
            mobile ? "text-2xl py-4 w-full text-center" : ""
        } text-white font-afacad font-medium hover:text-gray-200 transition-colors duration-300`}
    >
        {children}
    </Link>
);
